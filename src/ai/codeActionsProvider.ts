...
/**
 * Build FixExplainArgs from the active editor (selection or current line). Used when Fix/Explain are run from keybinding.
 */
export function getArgsFromActiveEditor(message?: string): FixExplainArgs | undefined {
    ...
    const startLine = Math.max(0, range.start.line - LINES_CONTEXT);
    const endLine = Math.min(document.lineCount - 1, range.end.line + LINES_CONTEXT);
    const contextBefore = document.getText(new vscode.Range(startLine, 0, range.start.line, 0));
    const contextAfter = document.getText(new vscode.Range(range.end.line + 1, 0, endLine + 1, 0));
    return {
        ...
        message: message ?? 'Fix or improve the selected code',
        code,
